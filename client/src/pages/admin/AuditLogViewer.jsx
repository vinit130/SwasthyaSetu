  ...
  const getActionBadge = (action) => {
    switch (action) {
      ...
      case 'UPDATE_INVENTORY_STOCK':
        return <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-100 text-amber-800 border border-amber-200">Stock Movement</span>;
      case 'RECORD_HOSPITAL_TREATMENT':
        return <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 border border-blue-200">Hospital Treatment</span>;
      case 'VIEW_REFERRAL_BY_TOKEN':
        return <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-purple-100 text-purple-800 border border-purple-200">Token Lookup</span>;
      case 'UPLOAD_MEDICAL_DOCUMENT':
        return <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-teal-100 text-teal-800 border border-teal-200">Document Upload</span>;
      default:
