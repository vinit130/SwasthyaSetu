        ...
        <form onSubmit={handleSearchSubmit} className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-xs border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
            />
          </div>
          <button
            type="submit"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {facilities.map((fac) => (
            <div
              key={fac._id || fac.id}
              className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-indigo-300 dark:hover:border-indigo-600 transition-all shadow-xs flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm">{fac.name}</h3>
                ...
                <div className="grid grid-cols-2 gap-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 text-xs mb-3">
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[10px]">Total Beds</span>
                    <strong className="text-slate-800 dark:text-slate-200">{fac.totalBeds || 0}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[10px]">Available</span>
                    <strong className="text-emerald-700 dark:text-emerald-400">{fac.availableBeds || 0}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[10px]">ICU Beds</span>
                    <strong className="text-purple-700 dark:text-purple-400">{fac.icuBeds || 0}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[10px]">ICU Available</span>
                    <strong className="text-indigo-700 dark:text-indigo-400">{fac.availableIcuBeds || 0}</strong>
                  </div>
                </div>

                {fac.servicesOffered && (
                  <div className="mb-4">
                    <span className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 block mb-1">Services:</span>
                    <div className="flex flex-wrap gap-1">
                      {fac.servicesOffered.map((s, idx) => (
                        <span key={idx} className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-0.5 rounded-md border border-slate-200/60 dark:border-slate-700">
                          {s}
                        </span>
                      ))}
                    </div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                {fac.emergencyContact ? (
                  <a
                    ...
                    className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 font-bold"
                  >
                    <Phone className="w-3.5 h-3.5" />
                    Emergency: {fac.emergencyContact}
                  </a>
                ) : (
                  <span className="text-slate-400 dark:text-slate-500">Emergency: 108</span>
                )}
