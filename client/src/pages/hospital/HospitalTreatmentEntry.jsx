...
export default function HospitalTreatmentEntry() {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="inline-flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 mb-2"
          >
      ...
      {success && (
        <div className="p-4 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 rounded-2xl border border-emerald-200 dark:border-emerald-800 flex items-center gap-3">
          <CheckCircle className="w-5 h-5 text-emerald-600 shrink-0" />
          <div>
            <p className="font-bold text-sm">Hospital Encounter Successfully Recorded!</p>
            <p className="text-xs text-emerald-700 dark:text-emerald-400">Patient care continuum updated. Redirecting to dashboard...</p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
        {error && (
          <div className="p-3 bg-red-50 dark:bg-red-950/40 text-red-700 dark:text-red-300 text-xs rounded-xl flex items-center gap-2 border border-red-200 dark:border-red-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          <div>
            <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Select Inpatient / Patient *</label>
            <select
              value={patientId}
          ...
          <div>
            <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Encounter Type</label>
            <select
              value={encounterType}
        ...
        <div className="space-y-3 text-xs">
          <div>
            <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Chief Complaint & Onset</label>
            <input
              type="text"
          ...
          <div>
            <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Primary Clinical Diagnosis *</label>
            <input
              type="text"
          ...
          <div>
            <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Hospital Treatment & Procedures Performed *</label>
            <textarea
              rows={3}
        ...
        <div className="space-y-3 text-xs">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm">Medications & Inpatient Orders</h4>
            <button
              type="button"
              onClick={handleAddMed}
              className="px-2.5 py-1 text-xs font-semibold text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/40 hover:bg-purple-100 dark:hover:bg-purple-900/50 rounded-lg border border-purple-200 dark:border-purple-800 flex items-center gap-1"
            >
          ...
          <div className="space-y-2">
            {prescriptions.map((med, idx) => (
              <div key={idx} className="grid grid-cols-1 sm:grid-cols-4 gap-2 p-2.5 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                <input
                  type="text"
                  placeholder="Medicine Name & Strength"
                  value={med.medicineName}
                  onChange={(e) => handleMedChange(idx, 'medicineName', e.target.value)}
                  className="px-2.5 py-1.5 border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 dark:text-slate-100"
                />
                <input
                  type="text"
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    className="flex-1 px-2.5 py-1.5 border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 dark:text-slate-100"
                  />
