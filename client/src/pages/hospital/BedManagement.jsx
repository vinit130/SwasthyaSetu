...
import { Bed, UserCheck, RefreshCw, CheckCircle, AlertCircle, X, Plus, Filter } from 'lucide-react';
...
export default function BedManagement() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default:
        return <span className="px-2 py-0.5 text-[10px] font-medium rounded-full bg-slate-100 text-slate-700 border border-slate-200">{status}</span>;
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="py-12 text-center text-slate-500 bg-white rounded-xl border border-slate-200 p-8">
            ...
            <div
              ...
              className={`p-3.5 rounded-2xl border transition-all cursor-pointer shadow-2xs flex flex-col justify-between ${
                bed.status === 'AVAILABLE'
                  ...
                  : bed.status === 'OCCUPIED'
                  ? 'bg-purple-50/50 border-purple-200 hover:border-purple-400 hover:bg-purple-50'
              ...
              <div>
                ...
                {bed.status === 'OCCUPIED' ? (
                  <p className="text-xs font-semibold text-purple-900 line-clamp-1">
                    {bed.patientName || bed.patientId?.name || 'Assigned Inpatient'}
                  </p>
                ) : (
              ...
              {selectedBed.status === 'AVAILABLE' && (
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white text-slate-800"
                  >
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 text-slate-800"
                  />
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <button
                      ...
                      className="flex-1 py-2 font-bold text-white bg-purple-700 hover:bg-purple-800 disabled:opacity-50 rounded-xl shadow-xs transition-colors cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-2 text-slate-600 hover:bg-slate-100 rounded-xl border border-slate-200 transition-colors cursor-pointer"
                    >
                      Sterilize / Maintenance
                    </button>
                  </div>
                </div>
              )}
              ...
              {selectedBed.status === 'OCCUPIED' && (
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full py-2.5 font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl shadow-xs transition-colors cursor-pointer"
                  >
              ...
              {selectedBed.status === 'MAINTENANCE' && (
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    disabled={saving}
                    onClick={() => handleUpdateBed('AVAILABLE', null)}
                    className="w-full py-2 font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors cursor-pointer"
                  >
